function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  // Create a helper function to escape HTML special characters
  ...
  // Initialize an array to store the highlighted segments of HTML content
  const highlightedSegments = [];

  // Loop through each object in plainTextPositions
  for (const position of plainTextPositions) {
    const { start, end } = position;

    // Extract the corresponding segment from plainText
    const plainTextSegment = plainText.slice(start, end + 1);

    // Find the start and end positions in htmlContent based on plainTextSegment
    const htmlStart = htmlContent.indexOf(escapeHTML(plainTextSegment));
    const htmlEnd = htmlStart + plainTextSegment.length;

    // Create the highlighted HTML segment
    const highlightedSegment = `<mark>${htmlContent.slice(htmlStart, htmlEnd)}</mark>`;

    // Push the highlighted segment into the array
    highlightedSegments.push({ htmlStart, htmlEnd, highlightedSegment });
  }

  // Sort the highlighted segments by their start position in descending order
  highlightedSegments.sort((a, b) => b.htmlStart - a.htmlStart);

  // Insert the highlighted segments back into htmlContent
  for (const segment of highlightedSegments) {
    const { htmlStart, htmlEnd, highlightedSegment } = segment;
    htmlContent =
      htmlContent.slice(0, htmlStart) + highlightedSegment + htmlContent.slice(htmlEnd);
  }
